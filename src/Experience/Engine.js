...
class Engine {
  ...
  async init({ skipScrollBind = false, labelContainer = document.body, initialScroll = null } = {}) {
    if (this.isInitialized) return

    document.body.classList.add('loading')

    try {
      // Start gallery immediately — no texture wait, fallback colors show at once
      await this.experience.init(this.scene, this.camera, labelContainer)

      this.scroll.init()

      // Restore saved scroll position before the very first rendered frame
      if (initialScroll !== null) {
        this.scroll.scrollCurrent = initialScroll
        this.scroll.scrollTarget  = initialScroll
        const restoredCameraZ = this.scroll.cameraZFromScroll(initialScroll)
        this.camera.position.z = restoredCameraZ
        this.experience.gallery.setImmediatePlaneVisibility(restoredCameraZ)
      }
      ...
      // Load textures in background — swaps in as each one arrives
  ...
  loadTexturesProgressively() {
    ...
    this.experience.gallery.getTextureSources().forEach(async (src) => {
      try {
        const texture = await loader.loadAsync(src)
        texture.colorSpace = THREE.SRGBColorSpace
        if (this.isRunning) this.experience.gallery.applyTextureBySource(src, texture)
      } catch {
        // keep fallback color
      }
    })
