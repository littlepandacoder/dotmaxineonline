...
class Gallery {
  ...
  applyTextureBySource(src, texture) {
    const idx = this.planeConfig.findIndex((p) => p.textureSrc === src)
    if (idx < 0) return
    const plane = this.planes[idx]
    if (!plane) return
    this.texturesBySource.set(src, texture)
    ...
    if (img?.width > 0 && img?.height > 0) {
      plane.userData.aspectRatio = img.width / img.height
      this.updatePlaneScale()
    }
  }

  setImmediatePlaneVisibility(cameraZ) {
    const blendData = this.getPlaneBlendData(cameraZ)
    if (!blendData) return
    const { currentPlaneIndex, nextPlaneIndex, blend } = blendData
    this.planes.forEach((plane, index) => {
      let opacity = 0
      if (index === currentPlaneIndex) opacity = 1 - blend
      if (index === nextPlaneIndex) opacity = Math.max(opacity, blend)
